...
### Upserting with a specific `id`

__Upserting__ also supports behavior where you would want to modify fields in a
document, even if the document `id` does not not exist.  
...
4. Device B downloads the same document from the backend API `{"firstName": "Adam"}` and upserts at t = 3, which overwrites the previous change synced at time = 
