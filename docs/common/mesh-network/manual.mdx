...
locations: 1234 and 7890. When a device identifies itself to the Big Peer, it gives it's 
location ID as part of the authentication callback. Before calling
`startSync()`, you can use a different sync group for each restaurant. Only
restaurants in the same sync group will sync with eachother.
:::
</p>
