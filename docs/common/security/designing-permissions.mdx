...
You can allow small peers to read documents in a collection *without permission
to write*. This can be useful in situations where you have restrictive user
roles, which can be identified by the document id. The asset in this situation
is a document, and Ditto ensures that only those authorized to create documents
with a particular id are able to syncronize those documents in a trustworthy
...
`userId` inside of the document `_id`. 
