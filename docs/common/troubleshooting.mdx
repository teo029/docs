...
If you are having trouble using Ditto, follow this guide.
...
* Ensure that you are only creating a fixed number of live queries. 
* Do not use `findAll()`. 
* Use smaller documents, and reference documents across collections (e.g., using foreign keys). *Ditto starts to slow down if documents are larger than 1MB. Documents larger than 5MB are not allowed. Instead, use the Attachments API.*

### General diagnostics

1. Set `DittoLogger.minimumLogLevel = DittoLogLevel.DEBUG` before you initialize `Ditto(identity)`.
1. Look at the logs. Do you see any helpful errors or warnings?
1. Verify that your app id is the same on all devices

### Multihop is not working

> Small Peer X subscribed to a ditto collection
> Small Peer Y subscribed to the same ditto collection 
> If I change something in the Big peer, both small peers (both IOS) are updated instantly. But if I switch off internet on Peer Y, Peer Y is not updated, while it is next to Peer X. 
...
1. Are you implementing the [authentication expiring soon](./security/authentication#creating-your-client) delegate?
1. Verify that your webhook provider name is [correctly copied in the Ditto portal](./security/authentication#login).
1. Verify that your authentication webhook is returning a payload that is formatted correctly according to the Ditto authentication specification. 
