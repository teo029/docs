...
As demonstrated in the following snippet, obtain the database debug logs _before_ Ditto is initialized so that any potential issues related to the file system access and authentication background tasks that run during initialization are tracked. 
...
When a subsription changes, you will see the following:
...
#### Did you copy your AppId and credentials correctly? 

If your AppId is wrong, you will see the following line:

```
AuthClient: failed to get JSON for auth response; e = reqwest::Error { kind: Decode, source: Error("expected value", line: 1, column: 1) 
```

If your playground token is wrong, you will see:
...
1. Send a POST request with a JSON stringified body to your server to ensure that the AWS-hosted Big Peer servers located in U.S. regions successfully send POST requests to your authentication server.
...
1. Keep a strong reference to the AuthClient for the duration of the Ditto object, otherwise the auth handler will become garbage collected at an inappropriate time.
...
account](https://portal.ditto.live) to chat with the support bot located in the lower
